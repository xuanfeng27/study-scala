...
object C06_模式匹配 {
  ...
  def main(args: Array[String]): Unit = {
     ...
     * for循环推导式
     */
    case class Lenovo(cpu: String, price: Double)

    // 变量定义中使用模式匹配
    val Array(k, v) = Array(1, 2)
    val List(r, p) = List(5, 6)
    val Lenovo(cpu, price) = Lenovo("intel 11", 5888.0)

    // for循环推导式中使用模式匹配
    val l1: Lenovo = Lenovo("intel 8", 4888.0)
    val l2: Lenovo = Lenovo("intel 9", 5888.0)
    val l3: Lenovo = Lenovo("intel 11", 6888.0)
    val l4: Lenovo = Lenovo("intel 12", 7888.0)
    val lst = List(l1, l2, l3, l4)

    for (Lenovo(cpu, _) <- lst) {
      println(cpu)
    }

    val mp = Map("a" -> 2, "b" -> 3, "c" -> 4)
    for ((k, v) <- mp) {
      println(k)
      println(v)
    }
